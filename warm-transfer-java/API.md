...
Submit data to `/transfer/initiate`.
...
```bash
curl -X POST http://localhost:5000/transfer/initiate \
  -H "Content-Type: application/json" \
  -d '{}'
```
...
Submit data to `/transfer/complete`.
...
```bash
curl -X POST http://localhost:5000/transfer/complete \
  -H "Content-Type: application/json" \
  -d '{}'
```
...
```bash
curl -X POST http://localhost:5000/hangup \
  -H "Content-Type: application/json" \
  -d '{}'
```
