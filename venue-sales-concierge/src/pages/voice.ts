 ...
 * in the page, no JWT minting. The assistant answers venue questions through
 * a webhook tool backed by this function's KV namespace + SQLDB
 * (/tools/lookup), so it always says exactly what the microsite says.
 */
export function renderVoicePage(venue: VenueData): string {
  return `<!DOCTYPE html>
<html lang="en">
<head>
<meta charset="UTF-8">
<meta name="viewport" content="width=device-width, initial-scale=1.0">
<title>Voice Concierge — ${esc(venue.venue.name)}</title>
<style>
  body { font-family: -apple-system, 'Segoe UI', sans-serif; margin: 0; background: #101048; color: #fefdf5; }
  .wrap { max-width: 720px; margin: 0 auto; padding: 2rem 1rem; }
  h1 { color: #00e3aa; }
  .panel { background: #2424a7; border-radius: 12px; padding: 1.5rem; margin: 1rem 0; }
  button { background: #3434ef; color: #fff; border: none; border-radius: 8px; padding: .7rem 1.3rem; font-size: 1rem; cursor: pointer; margin: .25rem; }
  button:disabled { opacity: .4; cursor: not-allowed; }
  button.danger { background: #eb0000; }
  #state { font-weight: 700; color: #00e3aa; }
  #error { color: #ff99a1; font-size: .9rem; min-height: 1.2rem; }
  .muted { color: #aed3f9; }
  a { color: #00e3aa; }
...
<div class="wrap">
  ...
  <p class="muted">${esc(venue.venue.name)} — talk right here in the browser. No number to dial.</p>
  ...
  <div class="panel">
    ...
    <div id="error"></div>
    <p class="muted">Ask about dates, capacity, catering menus, AV, parking, or book a site visit — answers come live from the venue KV store and SQL booking database.</p>
    <audio id="remoteAudio" autoplay controls></audio>
  </div>

  <p><a href="/">← Back to the venue site</a></p>
</div>
