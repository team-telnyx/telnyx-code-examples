...
import { INSIGHTS_HTML } from "./insightsHtml";
...
// Unified patient identity: in the demo, every channel converges on ONE actor
// keyed by the demo patient's phone. Returns null outside the demo setup.
function demoPatientActorName(): string | null {
  const demoPhone = process.env.DEMO_PATIENT_PHONE ?? "";
  return demoPhone ? customerIdForChannel("sms", demoPhone) : null;
}
...
export default {
  async fetch(req: Request, env: Env): Promise<Response> {
    ...
    if (url.pathname === "/insights" && req.method === "GET") {
      return new Response(INSIGHTS_HTML, {
        headers: { "Content-Type": "text/html; charset=utf-8" },
      });
    }

    // ── Open-tracking pixel: /email/open/{customerId}/{documentId} ─────
    const openMatch = url.pathname.match(/^\/email\/open\/([^/]+)\/([^/]+)$/);
    if (openMatch && req.method === "GET") {
      const customerId = decodeURIComponent(openMatch[1]);
      const documentId = decodeURIComponent(openMatch[2]);
      try {
        const stub = env.INBOX.idFromName(actorNameForCustomer(customerId));
        await stub.markResultsOpened(documentId);
      } catch {
        // best-effort tracking — a missed open must never fail the pixel
      }
    ...
    // ── Click tracking + patient portal: /email/click/{customerId}/{documentId} ──
    const clickMatch = url.pathname.match(/^\/email\/click\/([^/]+)\/([^/]+)$/);
    if (clickMatch && req.method === "GET") {
      const customerId = decodeURIComponent(clickMatch[1]);
      const documentId = decodeURIComponent(clickMatch[2]);
      let reference = "";
      try {
        const stub = env.INBOX.idFromName(actorNameForCustomer(customerId));
        const state = await stub.getDebugState();
        await stub.setState({ ...state, clicked_at: Date.now(), clicked_doc_id: documentId });
        const docs = await stub.listDocuments();
        reference = docs.find((d) => d.id === documentId)?.reference ?? "";
      } catch {
        // best-effort tracking — a missed click must never break the redirect
      }
      const portalUrl = process.env.PORTAL_URL ?? "https://portal.example.com";
      return Response.redirect(`${portalUrl}/results?ref=${reference}`, 302);
    }

    // ── Patient portal landing: /portal?ref=... ─────────────────────────
    if (url.pathname === "/portal" && req.method === "GET") {
      const ref = url.searchParams.get("ref") ?? "";
      const portalHtml =
        "<!DOCTYPE html><html><head><meta charset='UTF-8'><meta name='viewport' content='width=device-width, initial-scale=1.0'><title>Your Results — Telnyx</title>" +
        "<link rel=\"preconnect\" href=\"https://fonts.googleapis.com\"><link href=\"https://fonts.googleapis.com/css2?family=Inter:wght@400;600;700&family=Space+Grotesk:wght@600;700&display=swap\" rel=\"stylesheet\">" +
        "<style>body{font-family:Inter,sans-serif;background:#F7F7F2;margin:0;min-height:100vh;display:flex;align-items:center;justify-content:center}.card{background:#fff;border:1px solid #E4E4DC;border-radius:16px;padding:48px;max-width:480px;text-align:center;box-shadow:0 20px 60px rgba(0,0,0,.08)}.eyebrow{font-size:10px;font-weight:700;letter-spacing:.14em;text-transform:uppercase;color:#64645F}.brand{font-family:'Space Grotesk',sans-serif;font-size:28px;font-weight:700;letter-spacing:-.8px;margin:8px 0 20px}.ref{font-family:'Space Grotesk',sans-serif;background:#CCF9EE;border-radius:8px;padding:10px 16px;font-weight:700;display:inline-block}.msg{color:#64645F;font-size:14px;margin-top:20px;line-height:1.6}.dot{width:12px;height:12px;background:#00E3AA;border-radius:50%;margin:0 auto 16px}</style></head>" +
        "<body><div class='card'><div class='dot'></div><div class='eyebrow'>Telnyx Lab Results</div><div class='brand'>Your results are ready</div><div class='ref'>Reference: " +
        escapeHtmlPort(ref) +
        "</div><div class='msg'>Your lab results are ready and have been sent to your email by the clinic.<br>Keep this reference for any follow-up questions.</div></div></body></html>";
    ...
    if (url.pathname === "/api/appointment/book" && req.method === "POST") {
      ...
      const nextFriday = (): string => {
        ...
        return `${date} at 10:00 AM`;
      };
      const appointmentTime = body.appointment_time ?? nextFriday();
    ...
    if (url.pathname === "/api/demo/reset" && req.method === "POST") {
      ...
      const customers = await registryStub.listRegisteredCustomers();
      for (const c of customers) {
        const stub = env.INBOX.idFromName(c.customer_id);
        await stub.resetDemoState();
      }
      return Response.json({ reset: true, actors: customers.length });
    }
    ...
    if (url.pathname === "/api/demo/simulate-fax" && req.method === "POST") {
      ...
      const actorName = patientActor ?? actorNameForCustomer(faxNumber);
      const stub = env.INBOX.idFromName(actorName);
      const state = await stub.getDebugState();
      if (!state.customer_id) {
        await stub.bindVoiceCall({
          callerNumber: patientActor ? (process.env.DEMO_PATIENT_PHONE ?? faxNumber) : faxNumber,
    ...
    if (url.pathname === "/api/document/draft-email" && req.method === "POST") {
      ...
      const portalUrl = process.env.PORTAL_URL ?? "https://portal.example.com";
      const draftBody =
        `Hello,\n\nGood news — your lab results are ready.\n\n` +
        `Log in to the patient portal to view them:\n` +
        `${portalUrl}/results?ref=${doc.reference}\n\n` +
        `Your case reference is ${doc.reference}. If you have any questions, ` +
        `reply to this email or call us and mention your reference number.\n\n` +
        `Thank you.`;
      const draft = await stub.draftConfirmationEmail(body.document_id, draftBody);
      return Response.json({ draft });
    }

    if (url.pathname === "/api/email/insights" && req.method === "GET") {
      const registryStub = env.INBOX.idFromName("operator-default");
      const customers = await registryStub.listRegisteredCustomers();
      type OutboundEmail = Pick<
        MessageRow,
        | "id"
        | "conversation_id"
        | "status"
        ...
        | "ts"
      > & { to: string };
      const all: OutboundEmail[] = [];
      const docOpenedAtByConv = new Map<string, number>();
      for (const c of customers) {
        const stub = env.INBOX.idFromName(c.customer_id);
        const convs = await stub.listConversations({});
        const docs = await stub.listDocuments();
        const emailedToByConv = new Map(
          docs
            .filter((d) => d.emailed_to || d.patient_email)
            .map((d) => [d.conversation_id, (d.emailed_to || d.patient_email) as string]),
        );
        for (const d of docs) {
          if (d.opened_at && d.conversation_id) {
            docOpenedAtByConv.set(d.conversation_id, Number(d.opened_at));
          }
        }
        for (const cv of convs) {
          const msgs = await stub.listMessages(cv.conversation.id);
          for (const mv of msgs) {
            const m = mv.message;
            if (m.channel !== "email" || m.direction !== "outbound") continue;
            all.push({
              id: m.id,
              conversation_id: m.conversation_id,
              status: m.status,
              subject: m.subject,
              email_tracking_id: m.email_tracking_id,
              ts: m.ts,
              to: emailedToByConv.get(m.conversation_id) ?? c.customer_id,
            });
          }
        }
      }
      const sentOrFailed = all.filter(
        (m) => m.status === "sent" || m.status === "failed",
      );
      sentOrFailed.sort((a, b) => Number(b.ts) - Number(a.ts));
      ...
      for (const m of sentOrFailed) {
        if (m.status === "sent") totals.sent++;
        const trackingId = m.email_tracking_id;
        if (trackingId) totals.tracked++;
      }
      ...
      const byMessageId = new Map(
        sentOrFailed.map((m) => [
          m.id,
          {
            ...
            sent_at: Number(m.ts),
            status: m.status,
            tracked: Boolean(m.email_tracking_id),
            delivered_at: null as number | null,
            opened_at: null as number | null,
            clicked_at: null as number | null,
            bounced: false,
            failed: false,
          },
        ]),
      ...
      await Promise.all(
        sentOrFailed
          ...
          .map(async (m) => {
            ...
            try {
              const resp = await fetch(
                `${TELNYX_API}/email_messages/${encodeURIComponent(trackingId)}/events`,
                { headers: authHeaders(apiKey) },
              );
              if (!resp.ok) return;
              ...
              const row = byMessageId.get(m.id);
              if (!row) return;
              for (const e of data.data ?? []) {
                const type = String(
                  e.type ?? (e as { event_type?: string }).event_type ?? "",
                );
                const at = e.occurred_at
                  ? Date.parse(String(e.occurred_at))
                  : null;
                if (type.includes("deliver") && !row.delivered_at) {
                  row.delivered_at = at;
                  totals.delivered++;
                } else if (type.includes("open") && !row.opened_at) {
                  row.opened_at = at;
                  totals.opened++;
                } else if (type.includes("click") && !row.clicked_at) {
                  row.clicked_at = at;
                  totals.clicked++;
                } else if (type.includes("bounc") && !row.bounced) {
                  row.bounced = true;
                  totals.bounced++;
                } else if (type.includes("fail") && !row.failed) {
                  row.failed = true;
                  totals.failed++;
                }
              }
            } catch {
              // Per-message resilience: a single event-fetch failure (network or
              // payload) leaves that row unclassified instead of failing the
      ...
      for (const row of byMessageId.values()) {
        const docOpened = docOpenedAtByConv.get(row.conversation_id);
        if (docOpened && !row.opened_at) {
          row.opened_at = docOpened;
          totals.opened++;
        }
      }

      const deliveredDenominator = totals.delivered;
      const rates = {
        open_rate:
          deliveredDenominator > 0
            ? Math.round((totals.opened / deliveredDenominator) * 1000) / 10
            : 0,
        click_rate:
          deliveredDenominator > 0
            ? Math.round((totals.clicked / deliveredDenominator) * 1000) / 10
            : 0,
      };
      const messages = [...byMessageId.values()].map((r) => ({
        id: r.id,
        to: r.to,
        subject: r.subject,
        sent_at: r.sent_at,
      ...
      return Response.json({ totals, rates, messages });
    }
...
async function handleVoiceWebhook(req: Request, env: Env): Promise<Response> {
  ...
  if (eventType === "call.answered") {
    ...
    const greeting =
      "Hi, this is your lab results assistant. How can I help you today?";
  ...
  if (eventType === "call.transcription") {
    ...
    const transcriptionData = {
      transcript:
        (td.transcript as string) ??
        (payload.transcript as string) ??
        (payload.text as string) ??
        "",
      is_final: (td.is_final as boolean) ?? (payload.is_final as boolean) ?? true,
    };
    ...
    // Cross-actor lab context: the unified actor now holds the patient's
    // documents locally — draftReply's own context injection covers it, so
    // no per-customer fan-out is needed here (that loop added seconds of
    // cold-start latency to every utterance).
    const voiceBody = fragment;
...
async function handleAssistantLookup(req: Request, env: Env): Promise<Response> {
  ...
  if (doc) {
    ...
    if (doc.emailed_to && doc.email_sent_at) {
      ...
      result =
        `Reference ${doc.reference}: the lab document was received on ${receivedDate}, processed, and the results were sent to the patient on ${sentDate}. ` +
        `Tell the patient to log in to the patient portal using the link in their email to view the results. This line cannot provide the results themselves.`;
    } else {
      result =
...
async function handleFaxWebhook(req: Request, env: Env): Promise<Response> {
  ...
  if (!state.customer_id) {
    await stub.bindVoiceCall({
      callerNumber: patientActor ? (process.env.DEMO_PATIENT_PHONE ?? toNumber) : toNumber,
      callerLabel: "Hospital Intake",
...
async function handleMessagingWebhook(req: Request, env: Env): Promise<Response> {
  ...
  const payload = (event?.payload ?? {}) as Record<string, unknown>;
  const fromRaw = payload.from as unknown;
  const fromNumber =
    typeof fromRaw === "string"
      ? fromRaw
      : ((fromRaw as { phone_number?: string })?.phone_number ?? "unknown");
  const toRaw = payload.to as unknown;
  const toNumber = Array.isArray(toRaw)
    ? ((toRaw[0] as { phone_number?: string })?.phone_number ?? "")
    : typeof toRaw === "string"
      ? toRaw
      : ((toRaw as { phone_number?: string })?.phone_number ?? "");
...
async function sendApprovedMessage(
  ...
  if (msg.channel === "sms") {
    const raw = (msg.to ?? msg.customer_id ?? "").trim();
    const digits = raw.replace(/[^0-9]/g, "");
    if (!digits) throw new Error("sms send requires a recipient");
    const to = raw.startsWith("+") ? raw : `+${digits}`;
    await sendSms(apiKey, process.env.FROM_NUMBER ?? "", to, msg.body);
    return;
  }
  if (msg.channel === "email") {
    let linkedDoc: DocumentRow | null = null;
    if (msg.conversation_id) {
      const docs = await stub.listDocuments();
      linkedDoc =
        docs.find((d) => d.conversation_id === msg.conversation_id) ?? null;
    }
    const openPixelUrl =
      linkedDoc && msg.customer_id && process.env.PUBLIC_BASE_URL
        ? `${process.env.PUBLIC_BASE_URL}/email/open/${encodeURIComponent(msg.customer_id)}/${encodeURIComponent(linkedDoc.id)}`
        : null;
    const clickUrl =
      linkedDoc && msg.customer_id && process.env.PUBLIC_BASE_URL
        ? `${process.env.PUBLIC_BASE_URL}/email/click/${encodeURIComponent(msg.customer_id)}/${encodeURIComponent(linkedDoc.id)}`
        : null;
...
async function telnyxEmailSendReply(
  ...
  const htmlBody =
    "<html><body style=\"font-family:Arial,sans-serif;font-size:14px;line-height:1.6;color:#222;\">" +
    msg.body
      .split(/\n{2,}/)
      .map((p) => `<p>${escapeHtml(p).replace(/\n/g, "<br>")}</p>`)
      .join("\n") +
    (msg.openPixelUrl
      ? `<img src="${msg.openPixelUrl}" width="1" height="1" alt="" style="display:none">`
      : "") +
    "</body></html>";
  const trackedHtmlBody = msg.clickUrl
    ? htmlBody.replace(/https?:\/\/portal\.example\.com[^"<\s]*/g, msg.clickUrl)
    : htmlBody;
  ...
  const resp = await fetch(`${TELNYX_API}/email_messages`, {
    ...
    body: JSON.stringify({
      from: { email: fromEmail, name: "Telnyx Lab Results" },
      to: [to],
      subject: msg.subject ?? "Update on your lab document",
      text_body: msg.body,
      html_body: trackedHtmlBody,
    }),
  });
  if (!resp.ok) {
