...
<div class="header">
  ...
  <div class="header-tools">
    ...
    <a href="/insights" class="db-link">Email Insights ↗</a>
...
function renderDocuments(docs) {
  ...
  docs.forEach(d => {
    ...
    if (canAct) {
      ...
      if (d.opened_at) actions += '<span style="font-size:11px;font-weight:600;color:var(--inference-blue);margin-left:8px;">👀 opened ' + new Date(d.opened_at).toLocaleDateString([], {month:'short',day:'numeric'}) + '</span>';
...
function renderConversations(convs) {
  ...
  convs.forEach(c => {
    ...
    item.innerHTML = \`
      <div class="row1">
        <div class="label">\${maskCustomer(conv.customer_label || conv.customer_id)}</div>
...
document.addEventListener('click', function(e) {
  var btn = e.target.closest('.doc-accept, .doc-reject');
  if (!btn) return;
