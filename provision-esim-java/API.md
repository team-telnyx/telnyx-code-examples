...
## `GET /{simCardId}/status`

Retrieve data from `/{simCardId}/status`.
...
```bash
curl http://localhost:5000/{simCardId}/status
```

---

## `GET /group/{simCardGroupId}/list`

Retrieve data from `/group/{simCardGroupId}/list`.
...
```bash
curl http://localhost:5000/group/{simCardGroupId}/list
```
...
```bash
curl -X POST http://localhost:5000/sim-events \
  -H "Content-Type: application/json" \
  -d '{}'
```
