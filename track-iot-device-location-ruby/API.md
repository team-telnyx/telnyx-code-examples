...
## `GET /api/sim/:id/location`

Retrieve data from `/api/sim/:id/location`.
...
Retrieve data from `/api/sim/list`.
...
```bash
curl http://localhost:5000/api/sim/list
```

---

## `POST /webhooks/sim-location`
...
```bash
curl -X POST http://localhost:5000/webhooks/sim-location \
