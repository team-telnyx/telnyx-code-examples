...
│   ├── transform.py                  # Converts markdown tutorials → example folder
...
- `README.md` — structured overview + run docs (see README requirements below)
...
> Do **not** add `Dockerfile`, `Makefile`, or other deployment scaffolding; keep each
> folder focused on docs, code, and dependency/env files.
...
Converts a markdown tutorial into a deployable example folder with all required files.
...
a relative `./` or `../` path. External links
...
These links are auto-generated by `transform.py` via the `PRODUCT_LINKS` and `LANG_SDK_URL` mappings and the `build_resources()` helper.
