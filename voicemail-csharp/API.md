...
## `POST stop-recording/{callControlId}`

Submit data to `stop-recording/{callControlId}`.
...
```bash
curl -X POST http://localhost:5000stop-recording/{callControlId} \
  -H "Content-Type: application/json" \
  -d '{}'
```
...
## `POST hangup/{callControlId}`

Submit data to `hangup/{callControlId}`.
...
```bash
curl -X POST http://localhost:5000hangup/{callControlId} \
  -H "Content-Type: application/json" \
  -d '{}'
```
