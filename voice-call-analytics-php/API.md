...
Retrieve data from `/calls/{callControlId}/status`.
...
Submit data to `/calls/{callControlId}/hangup`.
...
```bash
curl -X POST http://localhost:5000/calls/{callControlId}/hangup \
  -H "Content-Type: application/json" \
  -d '{}'
```
