...
A regional logistics dispatcher runs a fleet of delivery drivers who coordinate time-sensitive handoffs through an SMS-based control center. If the dispatcher's identity fails mid-shift—say, during a high-volume morning batch—drivers could lose contact with the hub, miss pickup windows, and trigger cascading delays that cost the company thousands in SLA penalties. The actor IS the agent's identity. It comes into being when the dispatcher's agent is first provisioned, carrying its credentials and a promise of continuity from that first moment. It grows from a tentative, initializing presence into a trusted coordinator, renewing its access before the OAuth token lapses even while the server reboots mid-batch or the fleet goes quiet for an idle stretch. When a driver's authorization is revoked after a contract ends, the actor doesn't vanish—it records its own end, preserving the full history of every exchange so the company can audit the handoffs. The rest of this README is the API surface of that story.
